    ...
    const runSearchWithKey = async (apiKey: string): Promise<VisualCandidate[]> => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          log.warn('research.tavily_http_error', `HTTP ${res.status} from Tavily for "${keywords}"`, {
            keywords,
            status: res.status,
        ...
        log.debug('research.tavily_success', `Tavily returned ${candidates.length} candidates`, {
          keywords,
    ...
    try {
      if (this.explicitApiKey !== undefined) {
        if (!this.explicitApiKey) {
          log.warn('research.tavily_no_key', 'TAVILY_API_KEY is empty; skipping Tavily provider');
          return [];
        }
      ...
      if (!hasAvailableApiKeys('tavily') && !envConfig.TAVILY_API_KEY) {
        log.warn('research.tavily_no_key', 'TAVILY_API_KEY is not configured; skipping Tavily provider');
        return [];
      }
