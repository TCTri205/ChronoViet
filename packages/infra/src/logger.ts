...
const NON_SECRET_METRIC_PATTERN = /(token_?length|token_?count|total_?tokens?|prompt_?tokens?|completion_?tokens?|tokens?_?per_?sec)/i;

function isSecretKey(key: string): boolean {
  if (NON_SECRET_METRIC_PATTERN.test(key)) {
    return false;
  }
