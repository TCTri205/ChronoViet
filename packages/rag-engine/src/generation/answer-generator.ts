...
import { ClaimGrounder } from './claim-grounder.js';
...
export class AnswerGenerator {
  ...
  static async generate(
    ...
    const contextResult = ContextSynthesizer.assembleContext({
    ...
    const promptResult = PromptEngine.buildPrompt({
      query: request.query,
    ...
    const grounding = ClaimGrounder.groundClaims(answerText, contextResult.chunkMap);
    ...
    log.debug('rag.answer_generated', 'Historical answer generation completed', {
      ...
      citationCorrectness: grounding.citationCorrectnessScore,
    });
