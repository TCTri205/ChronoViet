...
> **ChronoViet Background Job Worker & Render Pipeline**  
...
## ⚙️ 2. Các Hàng Đợi Xử Lý Chính (BullMQ Queues)
...
### 4. HTTP Health & Metrics Probe Server (Port 3001)
* Tự động khởi chạy HTTP Server gọn nhẹ phục vụ kiểm tra sức khỏe và giám sát độ sâu hàng đợi:
  * `GET /healthz` (hoặc `/health`): Liveness probe báo cáo trạng thái worker.
  * `GET /readyz`: Readiness probe kiểm tra kết nối Redis (`ping -> PONG`).
  * `GET /metrics`: Prometheus metrics scraping trạng thái hàng đợi (`waiting`, `active`, `completed`, `failed`, `delayed`).
...
apps/render-worker/
├── src/
│   ├── index.ts                       # Worker Cluster Bootstrap & HTTP Probe Server (Port 3001)
│   ├── lib/
│   │   └── remotion-progress-parser.ts # Trích xuất frame và % tiến độ từ stdout Remotion CLI
│   ├── queues/
│   │   └── queue-manager.ts           # Quản lý kết nối BullMQ Queues & Prometheus queue metrics
│   ├── workers/
...
│   ├── runner.ts                      # Worker Benchmark Runner (--full, --jobs)
│   ├── tsconfig.json
...
## 🚀 5. Lệnh Phát Triển & Vận Hành (CLI Commands)
