...
* **Trạng thái Triển khai Hệ thống:**
  - **[✅ IMPLEMENTED RAG & INGESTION]:** Data Preprocessing & Ingestion Engine (Mô-đun 0) hoàn thiện 100% codebase & eval tại `packages/data-ingestion/src/`; Chrono-RAG Engine (Mô-đun 1) tại `packages/rag-engine/src/`. Cào tự động 100% 15 thời kỳ (`pnpm crawl:all`), Hybrid GraphRAG PostgreSQL pgvector + Relational Graph CTEs k=1,2 + BM25 FTS + RRF + Append-Only Audit Trail (`entity_audit_logs`).
  - **[✅ IMPLEMENTED AGENT ORCHESTRATION & GUARDRAILS]:** Multi-Agent Orchestrator (Mô-đun 2) hoàn thiện với LangGraph.js, Zod Schema v4.1, Research Provider Chain (`packages/agent-orchestrator/src/research/`), và 2 Automated Guardrail Gates: Folklore Guardrail Gate & NLI Entailment Hallucination Judge.
  - **[✅ IMPLEMENTED ENGINES & VLM]:** Engine Render Remotion 100% JSON-Driven (`packages/remotion-engine/src/`), 31 `LayoutMode`, 19 `TransitionType`, Zod Schema v4.1 runtime validation (`packages/shared-spec/src/schema.ts`). VLM Inspector Offline Quality Gate (`packages/vlm-inspector/src/`) lọc giấy phép bản quyền CC0, CC-BY, Public Domain và auto-fallback PURE_CODE.
  - **[✅ IMPLEMENTED TTS SERVICE & INFRA LAYER]:** Gói hạ tầng hợp nhất `@chronoviet/infra` (PostgreSQL connection pool, BullMQ Redis queues, Structured JSON logger, Prometheus telemetry, LLM API key rotator, BGE-M3 vector embeddings, Reranker client, VieNeu TTS Engine client) song hành cùng Python FastAPI ONNX Microservice (`services/vieneu-tts/`).
  - **[✅ IMPLEMENTED WEB APP & WORKER RUNTIME]:** Lớp ứng dụng `apps/web` (Next.js 14 App Router Monolith, NotebookLM Heritage Workspace UI/UX, Mobile Navigation Drawer, Live Multi-Node `/api/readyz` Health Monitor, 3 Sắc thái lời bình, REST API, SSE Stream, WebSocket Gateway) song hành cùng `apps/render-worker` (BullMQ queues, Asset pre-download `/media`, process isolation `CONCURRENCY=1`).
...
| **Tạo Giọng Đọc (TTS)** | Self-Hosted VieNeu Neural TTS (`vieneu.io`), Python FastAPI ONNX Engine + Node.js Client SDK (`@chronoviet/infra/tts`), Word Timestamps | **[✅ IMPLEMENTED]** (Phase 1 Microservice) | Giọng thuyết minh lịch sử truyền cảm, ngắt nghỉ chuẩn, sinh Word Timestamps cho chữ Karaoke. |
| **Dữ liệu & Tri thức (RAG)** | PostgreSQL-Powered GraphRAG (`pgvector` Dense BGE-M3 1024d + Relational Graph CTEs), SGK & Sử liệu cổ. | **[✅ IMPLEMENTED]** (100% Codebase & Real DB Eval) | Mô-đun 1: Hybrid GraphRAG (pgvector + Relational Graph CTEs k=1,2 + BM25 FTS + RRF + Reranker). Đã xác thực 100% trên DB thật (9,258 chunks, 82,849 quan hệ, 100 queries): Fact Precision 99.5%, Hallucination Rate 0%, Recall@10 100%, nDCG@5 0.940, p95 Latency 186.77ms. |
